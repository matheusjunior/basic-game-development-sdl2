...
void cMenu::createElements() {
}

void cMenu::adjustText() {
...
void cMenu::updatePosition(Text* t, int px, int py, int w, int h) {
	t->rect.x = px;
	t->rect.y = py;
	t->rect.w = w;
	t->rect.h = h;
...
Text* cMenu::loadFontAndSetPositionRef(int px, int py, int w, int h) {
...
Text cMenu::loadFontAndSetPosition(int px, int py, int w, int h) {
...
void cMenu::updateSelection(int direction) {
	...
	if (direction == MENU_UP) {
		if (previousSelected == 1 || previousSelected == 2) {
			updatePosition(m_gameMainMenuOpt1, SCREEN_WIDTH / 2 - 210 / 2 - m_rightArrowX, SCREEN_HEIGHT / 2 - 45, 150 + m_rightArrowW, 25);
	...
	if (direction == MENU_DOWN) {
		if (previousSelected == 2 || previousSelected == 3) {
			updatePosition(m_gameMainMenuOpt1, SCREEN_WIDTH / 2 - 210 / 2, SCREEN_HEIGHT / 2 - 45, 145, 25);
			updatePosition(m_gameMainMenuOpt2, SCREEN_WIDTH / 2 - 210 / 2, SCREEN_HEIGHT / 2 - 25, 130, 25);
			updatePosition(m_gameMainMenuOpt3, SCREEN_WIDTH / 2 - 210 / 2 - m_rightArrowX, SCREEN_HEIGHT / 2 - 5, 75 + m_rightArrowW, 25);
			m_gameMainMenuOpt1->displayText = "NEW GAME";
			m_gameMainMenuOpt2->displayText = "OPTIONS";
			m_gameMainMenuOpt3->displayText = "> EXIT";
...
Text* cMenu::getMainMenuOpt1() {
	return m_gameMainMenuOpt1;
}

Text* cMenu::getMainMenuOpt2() {
	return m_gameMainMenuOpt2;
}
Text* cMenu::getMainMenuOpt3() {
